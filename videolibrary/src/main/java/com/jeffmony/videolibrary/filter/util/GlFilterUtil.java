package com.jeffmony.videolibrary.filter.util;
...
public class GlFilterUtil {
    ...
    public static float[] createFilterMvpMatrix(@NonNull float[] vpMatrix,
        ...
        float scaleY;
        if (isPortraitVideo) {
            scaleX = transform.mSize.x;
            scaleY = transform.mSize.y * videoAspectRatio;
        } else {
            scaleX = transform.mSize.x * videoAspectRatio;
            scaleY = transform.mSize.y;
        }
        ...
        float translateY;
        if (isPortraitVideo) {
            translateX = transform.mPosition.x * 2 - 1;
            translateY = (1 - transform.mPosition.y * 2) * videoAspectRatio;
        } else {
            translateX = (transform.mPosition.x * 2 - 1) * videoAspectRatio;
            translateY = 1 - transform.mPosition.y * 2;
        }
        ...
        Matrix.rotateM(modelMatrix, 0, transform.mRotation, 0, 0, 1);
