...
public class UniformMatrix2fv extends ShaderParameter {
    ...
    public void apply(int glProgram) {
        if (mBuffer != null) {
            GLES20.glUniformMatrix2fv(getLocation(glProgram), mCount, mTranspose, mBuffer);
        } else {
            GLES20.glUniformMatrix2fv(getLocation(glProgram), mCount, mTranspose, mMatrix, mOffset);
        }
    }
}
